...
template <> \
void \
set_bcast<UintSet<Element, std::integral_constant<int, N>>, Element>( \
  std::vector<std::reference_wrapper<UintSet<Element, std::integral_constant<int, N>>>>& setRefs, \
  const Element max, \
  const int root, \
  const mxx::comm& comm \
) \
{ \
  ...
  auto blockSize = static_cast<uint32_t>(max + 63) / 64; \
  auto totalSize = blockSize * setRefs.size(); \
  auto bitSets = new ReduceType[totalSize](); \
  if (comm.rank() == root) {  \
    auto b = bitSets; \
    for (const auto& s : setRefs) { \
      memcpy(b, (*s.get()).b, blockSize * sizeof(ReduceType)); \
      b += blockSize;  \
    } \
  } \
  mxx::bcast(bitSets, totalSize, root, comm); \
  if (comm.rank() != root) { \
    auto b = bitSets; \
    for (auto& s : setRefs) { \
      memcpy((*s.get()).b, b, blockSize * sizeof(ReduceType)); \
      s.get().m_size = static_cast<Element>(set_size(*s.get())); \
      b += blockSize; \
    } \
  } \
} \
...
template <> \
void \
set_allunion_indexed<UintSet<Element, std::integral_constant<int, N>>, Element>( \
...
set_allgatherv<UintSet<Element, std::integral_constant<int, N>>, Element>( \
